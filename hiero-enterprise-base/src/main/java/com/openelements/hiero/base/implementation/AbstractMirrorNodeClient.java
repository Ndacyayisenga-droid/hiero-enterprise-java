...
public abstract class AbstractMirrorNodeClient<JSON> implements MirrorNodeClient {
    ...
    @Override
    public @NonNull Page<Contract> queryContractsBySolidityAddress(@NonNull final String solidityAddress) throws HieroException {
        Objects.requireNonNull(solidityAddress, "solidityAddress must not be null");
        final JSON json = getRestClient().queryContractsBySolidityAddress(solidityAddress);
        return getJsonConverter().toContractPage(json);
    }
