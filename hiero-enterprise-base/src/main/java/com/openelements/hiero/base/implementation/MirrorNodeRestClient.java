...
public interface MirrorNodeRestClient<JSON> {
    ...
    @NonNull
    default JSON queryContractsBySolidityAddress(@NonNull final String solidityAddress) throws HieroException {
        Objects.requireNonNull(solidityAddress, "solidityAddress must not be null");
        return doGetCall("/api/v1/contracts?solidity.address=" + solidityAddress);
    }
